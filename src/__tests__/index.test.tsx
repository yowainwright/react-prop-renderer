...
import { EndpointRenderer } from '../index';

describe('EndpointRenderer', () => {
	it('renders a string', () => {
		const { container } = render(<EndpointRenderer endpoint='test' />);
		const actual = container.querySelector('[data-value]').textContent;
		expect(actual).toEqual('test');
	});

	it('renders an array list items', () => {
		const { container } = render(<EndpointRenderer endpoint={['test', 'test2']} />);
	...
	it('renders an object', () => {
		const { container } = render(<EndpointRenderer endpoint={{ test: 'foo' }} />);
	...
	it('renders no data', () => {
		const { container } = render(<EndpointRenderer endpoint={null} />);
